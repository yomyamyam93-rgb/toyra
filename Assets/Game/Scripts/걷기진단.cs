...
public class 걷기진단 : MonoBehaviour
{
    ...
    bool 덧붙일까;      // 이번 누름에 아직 결과 줄을 안 붙였나
    ...
    void LateUpdate()
    {
        ...
        if (뗌 && 누름중)
        {
            ...
            // ★★상한에 닿아도 **덧붙일 칸은 새로 열어 둔다** (2026-08-09 렉 잡기).
            //   전엔 기록이 꽉 차면 아래 `기록[i] +=` 가 **마지막 칸에 영원히 덧붙었다.**
            //   그 문자열이 스윙마다 길어지는데 그걸 통째로 `Debug.Log` 로 뱉어서,
            //   플레이가 길어질수록 저절로 무거워졌다.
            if (기록.Count >= 최대) 기록.RemoveAt(0);
        ...
        if (덧붙일까 && 상태 != null && "" + 상태 == "쉼" && 지난상태 != null && "" + 지난상태 != "쉼" && 기록.Count > 0)
        {
            덧붙일까 = false;      // ★한 판에 한 줄만 — 안 막으면 같은 칸이 끝없이 자란다
            int i = 기록.Count - 1;
