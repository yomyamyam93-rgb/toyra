...
public class 제작창 : MonoBehaviour
{
    ...
    void 목록짜기()
    {
        ...
        // ★제작대 앞인가 — 가죽 제작법은 여기 있을 때만 뜬다 (2026-08-12)
        var 대 = 제작대.가까운것(transform.position, 닿는거리);

        foreach (var r in 제작법표.전부)
        {
            var 법 = r;                                  // 클로저용
            // 제작대가 필요한 것은 **대 앞에서만 목록에 오른다** — 안 그러면 왜 못 만드는지
            //   설명할 자리가 없어 「고장」으로 읽힌다. 자리가 곧 조건이다
            if (법.제작대필요 && 대 == null) continue;
            bool 도구 = 제작법표.도구있나(법);
            bool 재료 = 제작법표.재료있나(법);
            목록.Add(new 항목 {
                제목 = 법.이름,
    ...
    void 만들기(제작법 법)
    {
        ...
        if (법.세울것 == "모닥불") { 터놓기(); return; }
        // ★★제작대는 **바로 선다** — 불과 달리 「타는 과정」이 없다. 재료는 여기서 든다.
        //   인과는 그대로다: 망치를 들고, 나무·돌을 써서, 내가 세운다 (9-0)
        if (법.세울것 == "제작대") { 대세우기(법); return; }
    ...
    void 대세우기(제작법 법)
    {
        ...
        if (제작대.가까운것(앞, 2.2f) != null || 모닥불.가까운것(앞, 2.2f) != null)
        { 띄움("자리가 좁다"); return; }
        if (!제작법표.재료있나(법) || !제작법표.도구있나(법)) { 띄움("모자란다"); return; }

        제작법표.재료쓰기(법);              // 재료를 빼고 도구를 닳린다
        제작대.세우기(앞);
        띄움("제작대");
        열림 = false;
    }
