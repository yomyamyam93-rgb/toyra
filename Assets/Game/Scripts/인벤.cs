...
public class 인벤
{
    ...
    /// ★★몸에 걸친 것 — **칸 이름 → 아이템**. 칸을 코드에 박지 않으므로 늘려도 여기가 안 바뀐다 (5-7).
    ///   ☆손(`쥔것`)과 가방(`통들`)은 이미 제 길이 있어서 여기 안 들어온다 — 옷만 담는다
    public static readonly Dictionary<string, 아이템> 입은것 = new Dictionary<string, 아이템>();

    // ★★★**새 판이면 가진 것도 새로 시작한다** (2026-08-12 사용자 "몽둥이를 끼고있지 않는데
    //   이미 껴져있는 버그가 있는데?").
    //   ☆여기 것들은 전부 `static` 이라 **플레이를 껐다 켜도 남는다** (에디터에서 도메인
    //     리로드를 꺼 두면 더 확실히 남는다). 세상은 `world.Generate()` 로 새로 만드는데
    //     가방과 손만 지난 판 그대로였다 — 앞뒤가 안 맞는다.
    //   ☆`Boot` 가 `Stock.Clear()` 옆에서 부른다. 세상을 새로 짓는 자리와 같은 자리다.
    public static void 새판()
    {
        통들.Clear(); 통들.Add(주머니);
        주머니.것들.Clear();
        쥔것 = null;
