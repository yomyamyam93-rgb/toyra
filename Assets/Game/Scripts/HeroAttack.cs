...
public class HeroAttack : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (드는자세 != null)
        {
            ...
            드는자세.목표 = (클립쓴다 || 발차기짓) ? 0f : (싸우는중 ? 1f : 0f);   // ★맨손이면 팔 자세 코드가 돈다
    ...
    [Tooltip("발차기에서 허벅지를 드는 각 (°)")] public float 차기허벅 = -58f;
    [Tooltip("발차기에서 정강이를 펴는 각 (°)")] public float 차기정강 = 42f;
    [Tooltip("발차기에서 발목을 세우는 각 (°)")] public float 차기발목 = -20f;
    ...
    void 몸통스윙(float dt)
    {
        ...
        switch (state)
        {
            ...
            case State.예비:
            {
                ...
                if (발차기짓)
                {
                    // ★발차기의 예비는 **몸을 감는 게 아니라 무게를 옮기는 것**이다.
                    //   지지발로 체중을 넘기고 무릎을 조금 굽혀 찰 준비를 한다.
                    목표yaw = -차기열림 * 0.4f * w;
                    목표pitch = 2f * w;                      // 아주 조금만 숙인다
                    목표웅크림 = 차기지지굽힘 * w;
                    목표무게 = Mathf.Lerp(0.5f, 차기무게, w);
                }
                else
                {
                    목표yaw = -몸감기 * w; 목표pitch = -6f * w;
            ...
            case State.휘두름:
            {
                ...
                if (발차기짓)
                {
                    // ★★다리가 하는 일이라 몸통은 **반작용만** 한다. 스윙 아크(감았다 치기)도,
                    //   앞발 내딛기도 쓰지 않는다 — 차는 발은 앞으로 「나가는」 게 아니라 「들리는」 것이다.
                    //   ☆곡선은 `짓세기` 와 같은 봉우리(sin)다 — 다리와 몸통이 같이 정점을 찍는다.
                    float 봉 = Mathf.Sin(u * Mathf.PI);
                    목표yaw = 차기열림 * 봉;                 // 골반이 차는 쪽으로 열렸다 닫힌다
                    목표pitch = -차기젖힘 * 봉;              // ★상체가 뒤로 젖혀진다 (반작용)
                    목표웅크림 = 차기지지굽힘;               // 지지 다리로 버틴다
                    목표무게 = 차기무게;                     // ★앞으로 안 넘어간다
                    목표내딛음 = 0f;                         // ★★앞발이 나가지 않는다
                }
                else
                {
        ...
        if (골반뼈 != null && 웅크림 > 0.01f)
        {
            ...
            if (차기 > 0.001f)
            {
                if (오른허벅 != null) 오른허벅.localRotation *= Quaternion.Euler(차기허벅 * 차기, 0f, 0f);
                if (오른정강 != null) 오른정강.localRotation *= Quaternion.Euler(차기정강 * 차기, 0f, 0f);
                if (오른발목 != null) 오른발목.localRotation *= Quaternion.Euler(차기발목 * 차기, 0f, 0f);
            }
        }
