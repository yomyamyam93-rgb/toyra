...
public class HeroAttack : MonoBehaviour
{
    ...
    // ★0.9 — 사용자가 애니메이션 창에서 **앉는 구간을 0.9초까지**로 고쳤다 (2026-08-11).
    //   ☆이 값은 **클립을 고치면 같이 고쳐야 한다.** 어긋나면 앉다 말고 되풀이가 시작되거나,
    //     다 앉고도 한참 서 있는 것처럼 보인다.
    [Tooltip("갈무리 클립에서 앉는 구간의 길이 (초) — 이 뒤가 되풀이된다")]
    public float 갈무리앉는데 = 0.9f;
    ...
    // ★★★**손에 쥔 것이 있어야 무기가 보인다** (2026-08-12 사용자 "몽둥이를 끼고있지 않는데
    //   이미 껴져있는 버그가 있는데?").
    //   ☆무기 **그림**은 여태 인벤과 따로 놀았다 — `무기모델` 이 꽂혀 있으면 손 칸이 비어도
    //     늘 손에 들려 있었다. 수치는 `인벤.쥔것` 에서 읽으면서 그림만 딴 데를 봤다 (9-2).
    //   ☆편집 중에는 늘 보인다 — 씬에서 무기 자리를 눈으로 맞추려면 보여야 한다
    //     (2026-08-09 사용자 "무기위치 내가 보면서 조절할 수 있게해달라했는데").
    [Header("★등에 메기 (HeroCarry 가 켠다 — 줄을 끌 땐 두 손이 비어야 한다)")]
    [HideInInspector] public bool 등에멨나;
    [Tooltip("등에 멘 자리 (몸 기준)")] public Vector3 등짐자리 = new Vector3(-0.16f, 1.15f, -0.18f);
    [Tooltip("등에 멘 각도")] public Vector3 등짐기울임 = new Vector3(0f, 0f, 52f);

    void 무기자세()
    {
        if (무기 == null) return;

        bool 쥠 = !Application.isPlaying
                || (인벤.쥔것 != null && 인벤.쥔것.종 != null && 인벤.쥔것.종.무기);
        if (무기.gameObject.activeSelf != 쥠) 무기.gameObject.SetActive(쥠);
        if (!쥠) return;

        // ★등에 멘 동안은 손을 따라가지 않는다 — 자세 계산을 통째로 건너뛴다.
        //   그래야 씬에서 맞춰 둔 「멘 자세·쥔 자세」 값과 다투지 않는다 (소유권을 가른다)
        if (등에멨나)
        {
            무기.position = transform.TransformPoint(등짐자리);
            무기.rotation = transform.rotation * Quaternion.Euler(등짐기울임);
            모델손질();
            return;
        }
    ...
    void Update()
    {
        ...
        if (드는자세 != null)
        {
            ...
            // ★★**발차기면 팔을 안 든다** (2026-08-12) — 몽둥이 아크를 그리던 그 팔이
            //   발차기에도 그대로 나와서 「휘두르며 발 드는 것」이 됐다. 다리가 하는 일이니
            //   팔은 쉬는 자세에 둔다. 균형 자세가 필요하면 그때 손잡이로 뺀다.
            드는자세.목표 = 클립쓴다 ? 0f
                          : 발차기짓 ? 가드 * 가드세기      // ★찰 때는 팔이 올라와 균형을 잡는다 (몽둥이 아크가 아니다)
                          : (싸우는중 ? 1f : 0f);          // ★맨손이면 팔 자세 코드가 돈다
            ...
            드는자세.침 = 발차기짓 ? 0f                   // ★찰 때는 「든 자세」 그대로 — 내려치지 않는다
                        : state == State.휘두름 ? u * u
                        : state == State.여운 ? 1f
    ...
    void 들기그리기()
    {
        ...
        // ★★★**다 들면 다리를 걷기에 돌려준다** (2026-08-13 사용자 "머리위에 들고 걷기가 안돼는거").
        //   ☆드는 동안은 하체가 이 클립의 것이다 — 쪼그려 앉는 건 다리가 하는 일이니까.
        //   ☆그런데 다 든 뒤에도 붙들고 있으면 **다리가 마지막 포즈에서 얼어붙어** 걷지 못한다.
        //     상체(팔)는 계속 들고 있고, **하체만** 0층(걷기)에 넘긴다.
        bool 다들었다 = n >= 0.995f;
        if (하체층 > 0)
        {
            하체무게 = Mathf.MoveTowards(하체무게, 다들었다 ? 0f : 1f, Time.deltaTime * 14f);
            애니.SetLayerWeight(하체층, 하체무게);
            if (하체무게 > 0.001f) 애니.Play(들기해시, 하체층, n);
        }
    ...
    // ★★★**두 손이 차 있으면 못 때린다 — 그런데 컴포넌트를 끄면 안 된다** (2026-08-13 사용자
    //   "머리뒤로 손을 들어서 팻들 드는게아니라, 그냥 걷기모션에 …").
    //   ☆옛 코드는 새끼를 안을 때 `HeroCarry` 가 **이 컴포넌트를 통째로 껐다.** 그런데
    //     숙이는 자세(다리·골반·척추)를 만드는 `몸통스윙` 이 바로 이 안에 있다 —
    //     끄면 `줍기굽힘` 을 넘겨도 **받는 쪽이 죽어 있어서** 몸을 안 숙였다.
    //   → 컴포넌트는 켜 두고 **입력만** 막는다. 자세는 계속 돌고, 때리기만 안 된다.
    [HideInInspector] public bool 두손막힘;
...
#endif
        // ★두 손이 차 있으면 아무것도 안 눌린 것으로 본다 (자세 코드는 그대로 돈다)
        if (두손막힘) { 좌 = false; 우 = false; 뗌 = false; }
    }
    ...
    // ★★★★**맨손이면 몸짓이 갈린다 — 그림만 다르고 판정은 같다** (2026-08-12 사용자
    //   "맨손일때는 펀치를하든 발차기를 하든 다른 공격모션이 있어야할듯?" → "번갈아 나오는데 랜덤하게?").
    //   ☆여태 맨손이어도 **몽둥이를 휘두르는 자세** 그대로 팔을 돌렸다. 무기를 안 쥐면
    //     그림이 안 보이게 고친 지금(위 `무기자세`), 맨손 상태가 실제로 생겨서 더 티가 난다.
    //   ☆★**콤보가 아니다** (4장의 「콤보 없음」과 안 부딪힌다) — 피해도 사거리도 판정도
    //     똑같고 **보이는 몸짓만** 바뀐다. 오히려 순서를 외울 수 없어 연타가 정답이 안 된다.
    //   ☆같은 것이 연달아 안 나온다 — 완전 랜덤은 두세 번씩 겹쳐서 「고장 났나」로 보인다.
    [Header("★맨손 몸짓 — 주먹·발차기 (무기를 쥐면 안 쓴다)")]
    // ★★★★★**발차기는 「체임버 → 스냅 → 유지 → 리체임버 → 착지」다** (2026-08-12 사용자
    //   "어디 좀 보고 공부해서 좀 넣어봐"). 찾아보고 다시 짰다.
    //   ☆옛 코드는 **허벅지와 정강이를 sin 곡선 하나로 같이** 움직였다. 그러면 무릎을 들면서
    //     동시에 정강이가 펴져 **차는 게 아니라 다리를 휘두르는** 그림이 된다.
    //   ☆실제 발차기의 뼈대 (무술·애니메이션 공통):
    //     ① 무릎을 먼저 **높이 든다** — 정강이는 접힌 채다 (chamber). 여기가 핵심 포즈다
    //     ② 무릎 높이는 **그대로 두고** 정강이만 튕겨 뻗는다 (snap) — 여기가 임팩트
    //     ③ 임팩트를 **한 박 붙든다** (hit sustained)
    //     ④ 돌아올 때도 **정강이를 먼저 접고**(re-chamber) 나서 다리를 내린다
    //     ⑤ 착지에서 살짝 지나쳤다 돌아온다 (overshoot)
    //   ☆★**각도를 지어내지 않았다** (2026-08-11 교훈). 아래 기본값은 **이 리그의 클립에서
    //     실측한 값**이다 — 달리기 최대 무릎듦(허벅 80.6° 정강 69.6° 발 -69.3°)이 곧
    //     체임버 자세고, 선 자세(정강 28.5°)가 곧 「편 정강이」다.
    [Header("★발차기 자세 — 리그의 걷기·달리기에서 실측한 값")]
    [Tooltip("체임버 — 무릎을 들었을 때 허벅지 각(°). 달리기 최대듦이 80.6")]
    public float 체임버허벅 = 95f;
    [Tooltip("체임버 — 그때 정강이 접힘(°). 달리기 69.6 · 걷기 최대 83.7 (클수록 접힌다)")]
    public float 체임버정강 = 84f;
    [Tooltip("체임버 — 발목(°). 선 자세가 -87")]
    public float 체임버발 = -70f;
    [Tooltip("뻗음 — 임팩트 때 허벅지. 무릎 높이는 그대로 둔다")]
    public float 뻗음허벅 = 88f;
    [Tooltip("뻗음 — 정강이가 펴진다 (작을수록 곧다). 선 자세가 28.5")]
    public float 뻗음정강 = 6f;
    [Tooltip("뻗음 — 발끝을 세운다 (°)")]
    public float 뻗음발 = -52f;
    float 무릎듦, 뻗음정도, 가드;      // 0~1 — 아래 킥표가 민다

    // ★★★★**발차기는 스윙 위에 얹는 게 아니다** (2026-08-12 사용자 "기본 원래 공격모션에다가
    //   덧씌우니까 발차기가맞냐?"). 맞는 지적이었다 — `몸통스윙` 이 맨손짓과 상관없이 늘
    //   몽둥이 아크(몸감기 → 몸치기 · 앞발 내딛기 · 팔 스윙)를 돌리고, 발차기는 거기에
    //   **다리만 얹혀** 있었다. 실측으로도 드러났다: 발차기 클립의 `RightArm` 앞부분 키가
    //   주먹 클립과 **똑같았다**(14 / 14 / -39 / -37) — 휘두르는 팔이 그대로 들어 있었다.
    //   → 발차기일 때는 **스윙 아크를 아예 안 돌린다.** 아래 셋은 확실해서 코드가 쥔다:
    //     ① 앞발이 안 나간다 ② 팔이 몽둥이 아크를 안 그린다 ③ 몸통을 크게 안 감는다
    //   ☆나머지(젖힘·열림·지지굽힘)는 **눈으로 보고 정할 값**이라 손잡이로 뺐다.
    //     각도를 지어내지 않는다 (2026-08-11) — 화면에 띄워 놓고 정한다 (9-4).
    [Header("★발차기 몸놀림 — 스윙 아크를 안 쓴다")]
    [Tooltip("차는 동안 상체가 뒤로 젖혀지는 각 (°) — 발이 올라가는 반작용")]
    [Range(0f, 40f)] public float 차기젖힘 = 14f;
    [Tooltip("골반이 차는 쪽으로 열리는 각 (°) — 스윙처럼 크게 감지 않는다")]
    [Range(0f, 40f)] public float 차기열림 = 10f;
    [Tooltip("지지 다리가 굽는 정도 (0~1)")]
    [Range(0f, 1f)] public float 차기지지굽힘 = 0.35f;
    [Tooltip("차는 동안 무게가 실리는 쪽 (0 = 뒷발 · 1 = 앞발)")]
    [Range(0f, 1f)] public float 차기무게 = 0.15f;

    int 맨손짓, 지난맨손짓 = -1;      // 0 = 오른주먹 · 1 = 왼주먹 · 2 = 발차기
    float 차기;
    /// 지금이 발차기 몸짓인가 — 스윙 아크를 쓰면 안 되는 구간
    bool 발차기짓 => 맨손 && 맨손짓 == 2;

    // ★★★**킥표 — 발차기가 지나가는 여덟 포즈.** 봉우리 하나가 아니다.
    //   때는 `예비·휘두름·여운` 에서 그때그때 계산한다 → 3막 길이를 바꿔도 판정과 안 어긋난다.
    //   ☆줄 읽는 법: ①시동 ②체임버(무릎 최고·정강이 접힘) ③임팩트(정강이만 뻗음)
    //     ④임팩트 유지 ⑤리체임버(정강이 먼저 접음) ⑥내리는 중 ⑦착지(살짝 지나침)
    //   ☆**뻗음이 체임버보다 늦게 오는 것**이 핵심이다. 같이 가면 차는 게 아니라 휘두르는 것이다.
    readonly float[] 킥때   = new float[8];
    static readonly float[] 킥무릎 = { 0f, 0.40f, 1.00f, 0.95f, 0.95f, 0.85f, 0.15f,  0f     };
    static readonly float[] 킥뻗음 = { 0f, 0f,    0.05f, 1.00f, 0.95f, 0.10f, 0f,     0f     };
    static readonly float[] 킥젖힘 = { 0f, 0.30f, 0.65f, 1.00f, 1.00f, 0.60f, 0.15f, -0.06f  };
    static readonly float[] 킥가드 = { 0f, 1.00f, 1.00f, 1.00f, 1.00f, 0.85f, 0.40f,  0f     };
    [Tooltip("발차기 한 바퀴 길이 (초) — 클립 길이와 같아야 한다")] public float 킥길이 = 1.00f;
    [Tooltip("찰 때 두 팔이 올라오는 정도 (0~1) — 든 자세를 빌려 쓴다")]
    [Range(0f, 1f)] public float 가드세기 = 0.55f;
    float 킥t, 킥젖힘값;

    /// 킥표를 지금 시각으로 읽어 `무릎듦·뻗음정도·킥젖힘값·가드` 를 채운다
    void 킥곡선(float dt)
    {
        if (!발차기짓)
        {   // 발차기가 아니면 스르르 푼다 (자세가 눌러붙지 않게)
            무릎듦 = Mathf.MoveTowards(무릎듦, 0f, dt * 6f);
            뻗음정도 = Mathf.MoveTowards(뻗음정도, 0f, dt * 6f);
            킥젖힘값 = Mathf.MoveTowards(킥젖힘값, 0f, dt * 6f);
            가드 = Mathf.MoveTowards(가드, 0f, dt * 6f);
            킥t = 0f;
            return;
        }
        // ★시계는 **동작 시작부터의 초**다. 예비에 들어설 때 0 으로 되돌린다(맨손짓뽑기).
        킥t += dt;
        float P = 예비, S = 예비 + 휘두름, F = S + 여운;
        float T = Mathf.Max(F + 0.05f, 킥길이);
        킥때[0] = 0f;
        킥때[1] = P;
        킥때[2] = P + 휘두름 * 0.45f;        // ★체임버 — 휘두름 앞쪽 절반에서 무릎이 다 올라간다
        킥때[3] = S;                          // ★임팩트 — 휘두름이 끝나는 순간이 곧 맞는 순간
        킥때[4] = S + 여운 * 0.30f;          // ★한 박 붙든다
        킥때[5] = F;                          // ★리체임버
        킥때[6] = F + (T - F) * 0.45f;
        킥때[7] = T;

        float 초 = Mathf.Clamp(킥t, 0f, T);
        int i = 0;
        while (i < 킥때.Length - 2 && 초 > 킥때[i + 1]) i++;
        float 폭 = Mathf.Max(0.0001f, 킥때[i + 1] - 킥때[i]);
        float u = Mathf.SmoothStep(0f, 1f, Mathf.Clamp01((초 - 킥때[i]) / 폭));

        무릎듦   = Mathf.Lerp(킥무릎[i], 킥무릎[i + 1], u);
        뻗음정도 = Mathf.Lerp(킥뻗음[i], 킥뻗음[i + 1], u);
        킥젖힘값 = Mathf.Lerp(킥젖힘[i], 킥젖힘[i + 1], u);
        가드     = Mathf.Lerp(킥가드[i], 킥가드[i + 1], u);
    }

    /// 지금 자세 → 체임버 → 뻗음 순으로 **x축만** 섞는다. y·z 는 애니메이터 것을 지킨다.
    /// ★x 만 만지는 이유: 무릎은 한 축으로만 접힌다. 세 축을 다 덮으면 다리가 비틀린다.
    void 킥뼈(Transform 뼈, float 체임버x, float 뻗음x)
    {
        if (뼈 == null) return;
        var e = 뼈.localEulerAngles;
        float x = Mathf.Lerp(Mathf.DeltaAngle(0f, e.x), 체임버x, 무릎듦);
        x = Mathf.Lerp(x, 뻗음x, 뻗음정도);
        뼈.localEulerAngles = new Vector3(x, e.y, e.z);
    }
    ...
    void 맨손짓뽑기()
    {
        ...
        if (state == State.예비 && 맨손)
        {
            ...
            //
            // ★★★★**걷는 중에는 발차기를 뽑지 않는다** (2026-08-13 사용자 "걷기중에 발차기가
            //   나가서 오류나는거같은데, 걷기중에는 펀치만하도록 멈춰서 때리면 발차기").
            //   ☆걷는 중이면 아래 `하체목표` 가 0 이 되어 **다리를 걷기(0층)에 넘긴다.**
            //     그런데 발차기는 다리가 하는 동작이라, 상체만 찰 자세를 잡고 다리는 걷는
            //     **반쪽짜리 동작**이 나온다. 펀치도 발차기도 아닌 그것이 「끼어 있는 하나」다.
            //   → 걸을 땐 상체만으로 완결되는 주먹만 낸다. 서면 다시 번갈아 나온다.
            //     (지난맨손짓을 0 으로 남기므로, 멈추고 처음 치면 발차기가 나온다)
            bool 걷는중 = hero != null && hero.속도.magnitude > 걷기문턱;
            맨손짓 = 걷는중 ? 0 : (지난맨손짓 == 0 ? 2 : 0);   // 오른주먹 ↔ 발차기
            지난맨손짓 = 맨손짓;
            킥t = 0f;                              // ★킥표 시계는 여기서 0 이 된다
        }
        지난state = state;
    }
    ...
    /// ★★★**이번 그림을 클립이 그리나** (2026-08-12 사용자 "맨손 공격모션이 휘두르기랑
    ///   같은데? 펀치나 발차기가 아니고").
    ///   ☆`공격클립` 이 꽂혀 있으면 `몸통스윙` 이 **통째로 안 돌고** `HeroHold` 도 재운다.
    ///     맨손 몸짓을 거기 넣어 놨으니 한 번도 실행된 적이 없었다 — 9-2 그대로다.
    ///   ☆클립은 **무기를 휘두르는** 동작이라 주먹·발차기와 애초에 안 맞는다.
    ///     맨손이면 절차 쪽에 넘긴다.
    ...
    void 몸통스윙(float dt)
    {
        ...
        // ★발차기는 봉우리 하나가 아니라 **여덟 포즈를 지나간다** — 킥표가 정한다
        킥곡선(dt);
        차기 = 무릎듦;                    // 「발차기 중인가」를 읽는 자리들이 쓴다
        if (드는자세 != null)
        {
            드는자세.지르기 = (맨손 && 맨손짓 != 2) ? 짓세기 : 0f;
            드는자세.왼주먹 = 맨손짓 == 1;
        }
        ...
        // ★★★★**발차기는 3막 아크를 안 쓴다** (2026-08-12 사용자 "기본 원래 공격모션에다가
        //   덧씌우니까 발차기가맞냐?"). 위 switch 가 정한 몸통 값을 **통째로 갈아끼운다.**
        //   ☆세 상태(예비·휘두름·여운)를 각각 고치는 것보다 여기 한 곳이 낫다 — 새 상태가
        //     생겨도 발차기가 저절로 따라온다.
        //   ☆몸통은 **반작용만** 한다: 골반이 차는 쪽으로 열리고 상체가 뒤로 젖혀진다.
        //     앞발 내딛기는 끈다 — 차는 발은 앞으로 「나가는」 게 아니라 「들리는」 것이다.
        if (발차기짓 && (무릎듦 > 0.001f || Mathf.Abs(킥젖힘값) > 0.001f))
        {
            float 세기 = Mathf.Clamp01(Mathf.Abs(킥젖힘값) * 1.6f);
            목표yaw = 차기열림 * 킥젖힘값;
            목표pitch = -차기젖힘 * 킥젖힘값;
            목표웅크림 = 차기지지굽힘 * 세기;
            목표무게 = Mathf.Lerp(0.5f, 차기무게, 세기);
            목표내딛음 = 0f;
            빠르기 = state == State.휘두름 ? 40f : 18f;
        }
        ...
        if (골반뼈 != null && 웅크림 > 0.01f)
        {
            ...
            // ★발차기 — 선 자세 위에 **얹는다** (덮어쓰지 않는다). 그래야 웅크림·무게가 살아 있다
            // ★★★**얹지 않고 목표 자세로 간다** (2026-08-12). 옛 코드는 선 자세에 각을
            //   `*=` 로 얹었는데, 그러면 걷다 차느냐 서서 차느냐에 따라 결과가 달라지고
            //   무엇보다 **허벅지·정강이가 한 곡선으로 같이 움직여** 체임버가 안 생겼다.
            //   → 무릎(체임버)과 정강이(뻗음)를 **따로** 목표 각으로 몬다.
            if (무릎듦 > 0.001f)
            {
                킥뼈(오른허벅, 체임버허벅, 뻗음허벅);
                킥뼈(오른정강, 체임버정강, 뻗음정강);
