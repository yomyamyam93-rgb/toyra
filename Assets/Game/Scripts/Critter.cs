...
public class Critter : MonoBehaviour, IHittable
{
    ...
    // ★★★★**짐승도 제 인벤토리를 가진다** (2026-08-13 사용자 "먹이통이라고 지정하지말고,
    //   그냥 팻의 인벤토리이며 팻이 가지고 있는 아이템들인거지 (고기와 뼈같은건 아직 죽지않아서
    //   안나옴)").
    //   ☆**먹이 전용 통이 아니다.** 짐승이 가진 것 전부가 여기 있다 — 넣어 준 먹이도,
    //     나중에 죽으면 나올 고기·뼈도 같은 통이다. 살아서나 죽어서나 통은 하나다.
    //   ☆먹이를 넣어 두면 **천천히 먹는다.** 넣고 딴 일을 하면 된다 — 매번 클릭하지 않는다.
    //     기절해 엎어진 놈에게도, 매어 둔 놈에게도, 내 펫에게도 똑같이 넣는다.
    //   ☆★**먹을 것만 먹는다** — 그 밖의 것은 그냥 지고 있는다.
    //   ☆**큰 놈은 오래 씹는다** (정본 5-2 *"큰 종은 오래 걸리고 많이 먹는다"*). 무게가 정한다.
    //     새끼는 자주 먹어서 빨리 큰다.
    //   ☆통은 **처음 볼 때 만든다** — 세상에 짐승이 수백 마리라, 다 만들어 두면 낭비다 (9-4)
    [Header("가진 것 — 먹이를 넣어 두면 천천히 먹는다")]
    [Tooltip("한 입을 씹는 데 걸리는 시간 (초)")] public float 한입시간 = 6f;
    [Tooltip("무게가 씹는 시간을 얼마나 늘리나 — 큰 놈이 오래 걸리는 이유")]
    [Range(0f, 1f)] public float 무게가늘리는몫 = 0.3f;
    [Tooltip("들 수 있는 무게 (kg) — 여기에 몸무게 몫이 더해진다")] public float 통기본한도 = 3f;

    인벤 _가진것; float 씹은시간;

    /// 이 짐승이 가진 것 — 먹이도, 죽어서 나올 것도 여기 담긴다
    public 인벤 가진것 => _가진것 ??= new 인벤 {
        한도 = 통기본한도 + 종.무게 * 2f,
        이름 = 종.이름
    };
    /// 통을 이미 가졌나 — 안 만들어도 되면 만들지 않는다
    public bool 가진것있나 => _가진것 != null;

    /// 한 입에 걸리는 시간 — 무거울수록 오래, 새끼는 빨리
    public float 한입걸리는시간 => Mathf.Max(0.5f,
        한입시간 * (1f + 종.무게 * 무게가늘리는몫) * (새끼 ? 0.6f : 1f));

    // ★★★★**맞으면 달려든다** (2026-08-13 사용자 "팻이 뒤에서 맞으면 아무 반응없이 맞기만
    //   하는데 맞으면 애드가 나야하는거아닌가?" → 사용자 확정: **달려든다**).
    //   ☆여태 뒤에서 때려도 아무 일이 없었다 — 그러면 **뒤치기가 언제나 안전한 최적해**가 된다.
    //     헌법 5(*"싸울지 말지를 매번 고른다"*)가 서려면 때리는 쪽에도 대가가 있어야 한다.
    //   ☆★생포에도 값이 붙는다 — 기절시키려 여러 대 치는 동안 그놈이 달려든다.
    //   ☆기절·붙잡힘·매인 놈은 못 달려든다. 내 펫도 주인을 안 문다.
    //   ☆이것이 짐승이 스스로 하는 **행동**이라 확인을 받고 넣었다 (10-4-1)
    public void 맞음알림(IHittable 때린이)
    {
        if (!Alive || 기절중 || 잡힘 || 묶임) return;
        if (side == Side.내편) return;
        if (때린이 == null) return;
        target = 때린이;
        지금상태(상태.접근);
    }

    Animator 내애니; bool 애니찾음;

    /// 제 애니메이터를 켜거나 끈다 — 한 번만 찾아 들고 있는다 (9-4)
    void 애니멈춤(bool 켤까)
    {
        if (!애니찾음) { 내애니 = GetComponentInChildren<Animator>(true); 애니찾음 = true; }
        if (내애니 == null) return;
        if (내애니.enabled != 켤까) 내애니.enabled = 켤까;
    }

    /// ★가진 것 중 **먹을 것만** 한 입씩 먹는다 — 상태와 무관하다 (기절해 엎어져서도 먹는다)
    void 통에서먹기(float dt)
    {
        if (_가진것 == null || _가진것.것들.Count == 0) { 씹은시간 = 0f; return; }

        // ★먹을 것을 찾는다 — 돌·나무 같은 건 씹지 않고 그냥 지고 있는다
        아이템 먹을것 = null;
        for (int i = 0; i < _가진것.것들.Count; i++)
        {
            var x = _가진것.것들[i];
            if (x != null && x.종 != null && x.종.갈래 == 갈래.먹을것) { 먹을것 = x; break; }
        }
        if (먹을것 == null) { 씹은시간 = 0f; return; }

        씹은시간 += dt;
        if (씹은시간 < 한입걸리는시간) return;
        씹은시간 = 0f;

        // ★상한 것도 먹는다 — 다만 신뢰가 덜 오른다 (기획 5-7)
        float 배 = 먹을것.상했나 ? 0.5f : 1f;
        _가진것.꺼내기(먹을것.종.이름, 1);
        먹이받음(배);
    }
    ...
    // ★멀찍이 끌린다 (2026-08-13 사용자 "좀더 멀리서 떨어져서 끌려왔음해")
    [Range(0.6f, 8f)] public float 줄길이 = 4.4f;
    ...
    void Update()
    {
        ...
        // ★★★**붙잡혀 있을 때는 예외다** (2026-08-13 사용자 "팻이 머리위로 올라가질않아").
        //   사람이 짐승을 바닥 → 손 → 머리 위로 올리는데, 이 보정이 **매 프레임 땅으로
        //   끌어내려서** 아무리 올려도 발밑에 남았다. 드는 모션만 나오고 짐승은 그대로였다.
        //   ☆놓을 때 `HeroCarry.놓기()` 가 y 를 0 으로 되돌리므로 공중에 남지 않는다.
        if (!잡힘)
        { var 자리 = transform.position; float g = 땅격자.걷는높이(자리.x, 자리.z);
          if (자리.y != g) { 자리.y = g; transform.position = 자리; } }
        if (대표 && 무리 != null) 무리.식힘(dt);

        // ★★★**기절해 있으면 제 애니메이션을 멈춘다** (2026-08-13 사용자 "들었다 놓았을때,
        //   밧줄로 끌고가다 놓았을때 여전히 기절한 모션이여야하는데 그냥 서있네").
        //   ☆이 일을 `펫동작` 이 하게 해 뒀는데 **그 컴포넌트가 짐승에 안 붙어 있다** (실측).
        //     9-2 그대로 — 만들어 놓고 부르는 데가 없었다. 그래서 여기서 직접 쥔다.
        //   ☆기절은 **절차 모션**이다(`그로기몸`). 저작 클립이 같이 돌면 엎어진 몸 안에서
        //     다리가 걸어 다닌다. 깨어나면 다시 켠다.
        //   ☆죽은 놈은 건드리지 않는다 — 죽음 클립이 끝까지 돌아야 한다
        애니멈춤(!기절중);

        // ★먹이통은 상태와 무관하게 돈다 — 기절해 엎어져 있어도, 매여 있어도, 끌려가면서도 먹는다
        통에서먹기(dt);
    ...
    [Tooltip("붙잡혀 들려 있는 동안 흔들림을 얼마로 줄이나 — 0 이면 완전히 축 늘어진다")]
    [Range(0f, 1f)] public float 안겼을때흔들 = 0.12f;
    ...
    void 그로기몸()
    {
        ...
        // ★★**들려 있는 동안은 축 늘어진다** (2026-08-13 사용자 "흐느적흐느적").
        //   사람이 안고 흔드는 것과 제가 흔드는 것이 겹쳐서 두 겹으로 떨었다.
        //   기절해 안긴 놈은 제 힘으로 움직일 수 없다 — 거의 멈춘다
        if (잡힘) 세기 *= 안겼을때흔들;
        ...
        // ★★★★**스퀴시(크기 눌림)를 쓰지 않는다** (2026-08-13 사용자 "팻이 기절한 모션 넣는데
        //   스퀴시 또 사용한거, 스퀴시 쓰지말랬잖아").
        //   ☆이 파일 아래 `Squash` 주석에 **이미 적혀 있었다** — *"스케일 눌림은 안 쓴다
        //     (2026-08-09 사용자 「팻 쳐맞는 모션 만들고 왜 이상한걸 쓰냐」)"*. 피격에서 걷어낸
        //     것을 내가 기절에 다시 넣었다. 같은 지적을 두 번 받은 자리다.
        //   ☆크기는 **원래 값으로 유지**한다. 엎드린 그림은 회전(코가 처짐)과 자리(내려앉음)로만 낸다.
        float 눌 = Mathf.Lerp(1f, 기절눌림, 엎);       // ★자리를 내리는 데만 쓴다 (크기에는 안 쓴다)
