...
public class Critter : MonoBehaviour, IHittable
{
    ...
    [Tooltip("기절한 놈을 때리면 피해가 이만큼 배가 된다 — 봐줄 이유가 없다")]
    [Range(1f, 8f)] public float 기절중배 = 3f;
    ...
    {
        if (!Alive) return;
        // ★★★**기절한 놈을 더 패면 죽는다** (2026-08-12 사용자 "그상태에서 더패면 죽어버리는거고").
        //   엎어져 못 움직이는 것을 내려치는 것이니 봐줄 이유가 없다 —
        //   ☆이게 「죽일까 길들일까」를 매번 고르게 만든다 (기획 5-2). 기절시켜 놓고도
        //     한 대 더 치면 잡을 기회가 날아간다. 손을 멈추는 것 자체가 선택이 된다.
        if (기절중) d *= 기절중배;
        hp -= d;
