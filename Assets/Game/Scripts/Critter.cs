...
public class Critter : MonoBehaviour, IHittable
{
    ...
    // ★★★★**기절하면 체력이 조금 남는다** (2026-08-12 사용자 "한대 더치면 죽는게아니라,
    //   체력이 한 10~25프로 랜덤하게 남아서 맞으면죽게, 한대맞고 뒤지면 실수하면 어떻게하라고").
    //   맞는 지적이었다 — 앞서 「기절 중엔 피해 3배」로 넣었는데, 그러면 **한 대에 죽는다.**
    //   생포는 몇 분을 들여 몰아붙인 끝이라 **오클릭 한 번에 날아가면 안 된다.**
    //   ☆그렇다고 안 죽으면 「죽일까 길들일까」가 사라진다 — 그래서 **조금** 남긴다.
    //     몇 대는 견디되 계속 패면 죽는다. 얼마나 남았는지는 모르니 손을 멈추게 된다.
    //   ☆랜덤인 이유: 매번 같으면 「몇 대까지 괜찮다」를 외워 버려서 긴장이 사라진다.
    [Tooltip("기절할 때 남는 체력 (최대 체력에 대한 비율) — 최소")] [Range(0.02f, 0.5f)] public float 기절남는체력최소 = 0.10f;
    [Tooltip("기절할 때 남는 체력 — 최대")] [Range(0.02f, 0.6f)] public float 기절남는체력최대 = 0.25f;
    [Tooltip("기절 중 한 대에 깎이는 최대치 (최대 체력에 대한 비율) — 0 이면 상한 없음")]
    [Range(0f, 0.2f)] public float 기절한대한도 = 0.045f;
    ...
    public void 기절값먹임(float 값)
    {
        ...
        기절중 = true;
        기절든지 = 0f;
        // ★엎어지는 순간 체력이 조금만 남는다 — 몇 대는 견디되 계속 패면 죽는다.
        //   ☆**올리지는 않는다** — 이미 그보다 적게 남았으면 그대로 둔다 (때린 값이 사라지면 안 된다)
        float 남길 = 종.체력 * Random.Range(Mathf.Min(기절남는체력최소, 기절남는체력최대),
                                            Mathf.Max(기절남는체력최소, 기절남는체력최대));
        if (hp > 남길) hp = 남길;
        target = null;
    ...
    //   있을때 초록 테두리로 바뀌는데 그거 없애줄래? … 내가 생각한건 진짜 기절을 하는거야").
    //   옛 조건은 「넘어짐 or 기절 or 체력 35% 이하」였다. 체력만 깎여도 초록이 떠서,
    //   **아직 멀쩡히 서서 덤비는 놈**에게 「잡을 수 있음」 표시가 붙었다 — 그게 거짓말이다.
    //   ☆넘어짐(밀기로 자빠진 것)도 뺀다. 자빠진 건 잠깐 균형을 잃은 것이지 기절이 아니다.
    //   ☆체력을 20% 아래로 모는 것은 여전히 뜻이 있다 — 기절값이 쌓이기 전에 먼저 죽지
    //     않게 하려면 결국 지치게 만들어야 한다. 다만 그것만으로는 **못 잡는다.**
    public bool 지침 => Alive && side == Side.야생 && 기절중;
    ...
    void 휘두름진행(float dt)
    {
        ...
        if (전 < 휘두름시간)
        {
            ...
            //   ★★★**돌진은 「닿을 때까지」만 나간다** (2026-08-11 사용자 "돌진공격을하는거면
            //     그만큼 거리를 이동해야하는데 이동후 다시 제자리로 돌아가는게 말이안됌").
            //     이미 사거리 안인데도 계속 파고들면 **상대를 지나쳐 버리고**, 그러면 어느
            //     쪽으로든 되물러야 해서 「갔다가 돌아온다」가 된다. 돌진은 **거리를 좁히는
            //     행위**지 뚫고 지나가는 행위가 아니다.
            float 밀도;
            bool 이미닿음 = target != null && target.Alive
                          && 실루엣판정.닿나(target.T, transform.position, 닿는사거리, 0.1f);
            if (이미닿음) 밀도 = 0f;                       // 닿았으면 제자리에서 문다
            else if (휘두름t < 타격시점)
            {
    ...
    // ★★★★**먼저 덤비는 놈과 안 덤비는 놈을 가른다** (2026-08-11 사용자 "사슴이나
    //   다람쥐같은애들이 먼저 공격하는게 말이안돼서, 먼저공격안하는것들 선공하는것들
    //   구분해서 좀 넣어줬음해").
    //
    //   종 데이터는 **이미 옳았다** — 다람쥐 공격성 0.10("보면 도망간다"),
    //   사슴 0.15("먼저 안 덤빈다"), 늑대 0.70, 티라 0.95.
    //   그런데 `도망끔`(도망 기능 은퇴, 2026-08-06)이 켜지면서 `두려움값()` 이 **항상 0** 을
    //   돌려주고, 그러면 `덤빌까` 가 `공격성 > 0` 이 되어 **다람쥐까지 전부 덤볐다.**
    //   → 데이터를 실제로 쓰게 한다. 문턱 아래면 **표적을 아예 안 잡는다.**
    //
    //   ☆건드리면 덤빈다 — 때린 놈에게 반응조차 안 하면 그건 허수아비다.
    //   ☆새끼가 위험하면 어미는 덤빈다 (기획 5-2 "무리가 살아 있으면 방해한다").
    //   ☆은퇴는 스위치 — `선공가르기 = false` 면 옛날처럼 전부 덤빈다.
    public static bool 선공가르기 = true;
    [Tooltip("공격성이 이 값 이상이면 먼저 덤빈다 — 아래면 건드려야 반응한다")]
    public static float 선공문턱 = 0.5f;
    [Tooltip("맞은 뒤 이 시간 동안은 순한 놈도 덤빈다 (초)")]
    public static float 앙갚음시간 = 12f;
    float 건드린때 = -99f;

    /// 먼저 덤비나 — 사나운 종이거나 · 내가 건드렸거나 · 제 새끼가 위험하거나
    public bool 선공하나 =>
        !선공가르기
        || 종.공격성 >= 선공문턱
        || Time.time - 건드린때 < 앙갚음시간
        || (무리 != null && 무리.새끼위험 && !새끼);
    ...
    IHittable 표적찾기()
    {
        // ★★순한 놈은 **표적을 아예 안 잡는다** — 그래야 다가오지도, 쫓지도 않는다.
        //   (상태 기계 안에서 막으면 「경계 → 어슬렁 → 다시 경계」로 맴돈다)
        if (side == Side.야생 && !선공하나) return null;
    ...
    {
        if (!Alive) return;
        // ★★★★**기절 중엔 한 대에 깎이는 양에 상한이 있다** (2026-08-12).
        //   체력을 10~25% 남기는 것만으로는 모자랐다 — 실측: 다람쥐(42)는 남는 게 4~11 인데
        //   몽둥이가 14 라 **무조건 한 대에 죽었다.** 「실수 한 번에 날아가지 않게」가 목적인데
        //   작은 놈일수록 그 목적이 깨진 것이다.
        //   → 무기가 무엇이든 **최대 체력의 일부까지만** 깎인다. 그러면 크기와 무관하게
        //     몇 대는 견디고, 계속 패면 죽는다.
        //   ☆이래야 「죽일까 길들일까」가 살면서도 오클릭 한 번이 몇 분을 날리지 않는다.
        if (기절중 && 기절한대한도 > 0f) d = Mathf.Min(d, 종.체력 * 기절한대한도);
        hp -= d;
        건드린때 = Time.time;      // ★건드리면 순한 놈도 덤빈다 (`선공하나`)
