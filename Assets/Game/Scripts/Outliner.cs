...
public class Outliner : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // ★★★★**전체 훑기를 「자식 수」에서 떼어낸다** (2026-08-11 실측).
        //
        //   자식 수 문은 「아무것도 안 생겼을 때」만 막는다. 그런데 야생은 **쉬지 않고**
        //   스폰되니 문이 사실상 늘 열려 있었다 — 결과가 **1.5초마다 93ms · GC 1.8MB** 다
        //   (렉재기 로그로 확인. 세계 렌더러 11만 개를 `FindObjectsByType` 로 뜨는 값).
        //   ☆푸시(`새것`)로 자식 수를 맞춰 봐도 소용없었다 — 스폰은 한 번에 끝나지 않고
        //     그 뒤로도 오브젝트가 더 붙어서 셈이 또 어긋난다. **문의 판별법 자체가 틀렸다.**
        //
        //   → 새로 생긴 것은 **만든 쪽이 알려 준다**(`새것`). 전체 훑기는 아무도 안 알려준
        //     것을 줍는 **안전망**이라, 20초에 한 번이면 된다. 1.5초 → 20초로 13배 줄었다.
        강제cd -= 다시훑기;
        if (강제cd > 0f) return;
        강제cd = 강제훑기;
        지난자식수 = 씬바뀜.자식수합();
