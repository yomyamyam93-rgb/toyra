...
public class WorldGen : MonoBehaviour
{
    ...
    static readonly (Land 땅, int 권역수, int 칸수, float 먼곳부터, Color 바닥, float 바닥세기)[] 테마표 =
    {
        ...
        // ★★블록은 **황야**다 (2026-08-11 사용자가 「블록황야」 자산을 만들어 왔다 —
        //   벽돌 마른나무·선인장·대나무·다육·석조 구조물). 바닥이 초록이면 선인장이 안 산다.
        //   → 마른 흙빛으로. 소품은 `블록프리팹` 에 꽂혀 있다 (비면 상자로 짓는다).
        (Land.블록,     2, 4, 0.45f, new Color(0.62f, 0.50f, 0.34f), 0.80f),   // 마른 황야의 흙빛
    ...
    // ★★★23 — **같은 함정에 두 번 빠졌다** (2026-08-11 사용자 "동굴 구조물 위에는 이제
    //   나무 세우지말랬잖아"). 값을 고치는 사이에 `Generate()` 를 돌려 씬 정본이 22 로
    //   올라갔고, 그 뒤에 쓴 것들이 전부 막혀 있었다. 실측으로 확인한 안 먹은 값들:
    //     뚜껑나무 0.07(0 으로 썼는데) · 뚜껑잔디 0.25(0.55) · 살두께 1(2) ·
    //     안쪽벽깎기 2(0) · 작은굴규모 9(7)
    //   ☆교훈: **정본 블록의 값을 건드리면 번호부터 올린다.** 순서를 바꾸면 안 잊는다.
    const int 정본지금 = 23;
    ...
    void 정본맞추기()
    {
        ...
        // ★★뚜껑 위 — **나무는 절대 안 심고, 풀은 많이** (2026-08-11 사용자 두 번 말함:
        //   "동굴 천장에 나무는 박지 말아줘, 그냥 잔디 재질만 좀 입혀줘, 불규칙하게" ·
        //   "동굴 구조물 위에는 이제 나무 세우지말랬잖아, 뚜껑 위쪽은 풀로 좀 많이 덮히게끔")
        뚜껑잔디 = 0.78f;       // 0.55 → 0.78 · "좀 많이 덮히게"
        뚜껑잔디얼룩 = 16f;     // 얼룩을 조금 크게 — 덩어리로 덮인다
        뚜껑나무 = 0f;          // ★나무 없음
        ...
        살두께 = 2;
        안쪽벽깎기 = 0;
        입구폭 = 1.9f;          // ★2.8 → 1.9 (너비 5.6m → 3.8m). 좁아야 「굴 입구」로 읽힌다
        입구트임 = 9f;
        굴가장자리 = 90f;
        굴집비움 = 150f;
        정본 = 정본지금;
    ...
    // ★★★**입구가 「굴」로 읽혀야 한다** (2026-08-11 사용자 "동굴입구가,, 좀 알아보기
    //   어려운데, 동굴들이 다 입구가 넓기도하고").
    //   원인이 둘이었다: ①통로 반경이 2.8m(너비 5.6m)라 「틈」이 아니라 「바위 사이 공터」로
    //   읽혔다 ②입구까지 뚜껑이 덮여 있어, 위에서 내려다보는 시점인데 **구멍이 안 보였다.**
    //   → 좁히고, **입구 목의 뚜껑을 벗긴다.** 위에서 검은 홈이 보이면 즉시 굴로 읽힌다.
    //   ☆좁히는 건 덤으로 헌법 7 을 살린다 — 큰 놈이 못 들어오는 병목이 된다.
    [Tooltip("입구 통로의 반지름 (m) — 너비는 두 배. 좁을수록 굴처럼 읽힌다")]
    [Range(1.2f, 4f)] public float 입구폭 = 1.9f;
    [Tooltip("입구에서 이만큼은 뚜껑을 안 씌운다 (m) — 위에서 검은 구멍이 보이게")]
    [Range(0f, 24f)] public float 입구트임 = 9f;
    ...
    void 동굴터(Vector3 c, int 규모고정 = -1)
    {
        ...
        살.RemoveWhere(k =>
        {
            ...
            return 앞 > -2f && 앞 < 뚫는길이 && (q - 바깥벡 * 앞).magnitude < 입구폭;
        });
        ...
        foreach (var kv in 천장)
        {
            ...
            // ★★입구 목은 **뚜껑을 안 씌운다** — 위에서 내려다보는 시점이라, 덮여 있으면
            //   벽면에 난 구멍이 안 보여서 그냥 바위 홈으로 읽힌다. 벗겨 두면 검은 슬롯이
            //   바위 덩어리를 파고든 게 보여서 「여기로 들어간다」가 한눈에 읽힌다.
            //   ☆안쪽은 여전히 덮여 있어서 「굴」이다 — 입구만 트인 것이다.
            var q목 = new Vector3(kv.Key.ix * 칸, 0f, kv.Key.iz * 칸) - 입구p;
            float 앞목 = Vector3.Dot(q목, 바깥벡);
            bool 입구목 = 앞목 > -2f && 앞목 < 입구트임 && (q목 - 바깥벡 * 앞목).magnitude < 입구폭 + 1f;
            if (!입구목)
