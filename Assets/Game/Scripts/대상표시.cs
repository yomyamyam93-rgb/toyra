...
public class 대상표시 : MonoBehaviour
{
    ...
    // ☆2026-08-11 에 이걸로 잡았다 — 「찾기는 되는데 표시만 안 뜬다」였고, 원인은
    //   `대상표시` 가 **점 하나로 재는 옛 코드를 따로 갖고 있던 것**이었다.
    //   지금은 `Harvest.찾기()` 를 그대로 부른다. 다시 이상해지면 여기를 켠다.
    [Tooltip("외곽선이 왜 안 보이는지 콘솔에 남긴다 — 잡고 나면 끈다 (9-4)")]
    public bool 진단 = false;
    float 진단마지막;
