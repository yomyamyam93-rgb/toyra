...
public class PixelScreen : MonoBehaviour
{
    ...
    [Tooltip("휠을 축소 1 아래로 내렸을 때 갈 수 있는 최대 해상배수 — 2 면 두 배 넓게 본다")]
    [Range(1, 4)] public int 최대해상배수 = 3;
    [Tooltip("휠로 줌한다")]
    ...
    // ★★★**여유는 건너는 동안만 필요하다** (2026-08-06 실측). 오버스캔 2.5 로 멈춰 있으면
    //   1920×1080 화면을 **4802×2702 로 그리고**, 거기에 실루엣·물체 마스크 카메라가 같은
    //   크기로 또 찍어 **한 프레임에 41메가픽셀 = 1080p의 19.8배**가 된다.
    //
    //   재 봤다 (오버스캔만 바꾼 A/B):
    //     2.5 → 46.2ms (21.7fps) · rt 4802×2702
    //     1.1 → 38.0ms (26.3fps) · rt 2114×1190      ← 8ms 를 그냥 돌려받는다
    //
    //   ☆그런데 그냥 낮추면 **칸을 건널 때 가장자리가 빈다** — 그게 오버스캔의 존재 이유다.
    //   → 그래서 **멈춰 있을 때만 좁게** 그린다. 건너는 0.15초 동안은 예전처럼 넓게 그린다.
    //     축소가 바뀌는 순간은 어차피 판을 다시 만드는 자리라(아래 `만들기` 조건) 덧드는 비용이 없다.
    [Tooltip("멈춰 있을 때 쓰는 여유 — 건너는 동안만 「오버스캔」으로 넓힌다")]
    [Range(1f, 2.5f)] public float 평소오버스캔 = 1.15f;

    /// 지금 판이 실제로 쓰고 있는 여유 (평소 ↔ 건널 때)
    float 쓰는오버스캔;
    ...
    // ★**칠하는 두께**다. 실루엣을 미는 거리(`Outliner.부풀리기`)와는 **독립**이다
    //   (2026-08-05 사용자 확정). 둘을 같게 두면 선이 몸에 붙어 속이 차고, 부풀리기를
    //   줄이면 선이 몸 안쪽으로 파고든다 — 어느 쪽을 쓸지는 눈으로 고른다.
    [Range(1f, 6f)] [Tooltip("선을 몇 픽셀 두께로 칠하나 (부풀리기와 따로 논다)")]
    public float 외곽선두께 = 1f;
    ...
    // ★★번호를 올리면 **플레이할 때마다 코드 값이 인스펙터 값을 덮는다.**
    //   2026-08-06 에 5 로 올렸다가 사용자가 "내가 설정값 바꾸는데 왜 자꾸 다시 돌려놓냐" —
    //   맞는 지적이라 되돌렸다. `평소오버스캔` 은 **새 필드라 어차피 코드 기본값(1.15)으로
    //   들어오므로** 정본을 건드릴 이유가 처음부터 없었다.
    //   ☆여기를 올리는 건 「씬에 잘못 저장된 값이 코드를 이기고 있을 때」뿐이다.
    const int 정본지금 = 4;
    ...
    void 정본맞추기()
    {
        ...
        // ★멈춰 있을 때는 이만큼만 — 실측으로 8ms 를 돌려받는 자리다 (위 주석의 A/B)
        평소오버스캔 = 1.15f;
        최소축소 = 1;
        최대축소 = 10;
        ...
        // ★★★**스냅을 끈다** (2026-08-06 사용자 — "어차피 자글자글한거 카메라도 그냥
        //   부드럽게 움직이게해주라").
        //   스냅은 「픽셀 크롤」을 없애려고 카메라를 픽셀 격자에 딱 붙이고, 버린 소수점만큼
        //   화면을 되미는 방식이다. 이론상 또렷하면서 부드러워야 하는데 — **실제로는
        //   되미는 몫이 화면 픽셀 단위로 반올림돼서** 걸을 때 카메라가 미세하게 튄다.
        //   ☆그리고 우리 화면은 애초에 픽셀이 굵고 무늬가 잘아서, 스냅으로 얻는 또렷함이
        //     눈에 안 띈다. **얻는 건 안 보이고 잃는 건 보이면 끄는 게 맞다.**
        //   기능은 남겨 둔다 — 인스펙터에서 `스냅` 을 켜면 예전 그대로다.
        스냅 = false;
    ...
    void 만들기()
    {
        ...
        if (쓰는오버스캔 <= 0f) 쓰는오버스캔 = 오버스캔;      // 처음 한 번은 넉넉히
        float 넓게 = Mathf.Max(1f, 쓰는오버스캔);
    ...
    void LateUpdate()
    {
        ...
        //
        // ★★★**축소 1 아래로도 나갈 수 있게 이어 붙였다** (2026-08-06 사용자 "픽셀당미터
        //   40으로 바꿨는데 화면 더 축소할 수 있게").
        //   보이는 폭 = 렌더 가로 ÷ (축소 × 픽셀당미터) 이므로, `픽셀당미터 40` 에서 축소 1 은
        //   1920/40 = **48m** 가 한계다. 더 넓히는 길은 둘뿐인데
        //     ①픽셀당미터를 낮춤 → 그림의 결(굵기)이 바뀐다
        //     ②**크게 그려 줄여 깔기**(`해상배수`) → 결은 그대로, 픽셀만 잘아진다
        //   → 휠을 1 밑으로 내리면 ②로 넘어간다. `줌칸 0` = 해상배수 2 (두 배 넓게),
        //     `줌칸 -1` = 해상배수 3. 대가는 그리는 픽셀 수다 (2배 넓으면 4배).
        if (켬 && 휠로줌)
        {
            ...
            {
                int 줌칸 = (해상배수 > 1) ? (2 - 해상배수) : 축소;      // 배수2 → 0, 배수3 → -1
                줌칸 = Mathf.Clamp(줌칸 + (sc > 0f ? 1 : -1),
                                  2 - Mathf.Max(1, 최대해상배수), 최대축소);
                if (줌칸 >= 1) { 축소 = 줌칸; 해상배수 = 1; }
                else { 축소 = 1; 해상배수 = 2 - 줌칸; }
            }
        }
#endif
        ...
        // ★칸을 건너기 시작하면 **먼저 여유를 넓힌다** — 판을 다시 만드는 그 프레임에
        //   같이 넓혀야 건너는 중간에 가장자리가 안 빈다
        if (축소 != 지난축소) 쓰는오버스캔 = 오버스캔;
        ...
        // ★다 건넜으면 여유를 다시 좁힌다 (한 프레임 재생성 ↔ 매 프레임 8ms — 후자가 훨씬 비싸다)
        if (보이는축소 == 축소 && 쓰는오버스캔 > 평소오버스캔 + 0.001f)
        {
            쓰는오버스캔 = Mathf.Max(1f, 평소오버스캔);
            만들기();
            if (!켬 || mat == null || cam == null) return;
        }
